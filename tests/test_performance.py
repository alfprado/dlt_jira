...
class TestPerformance:
        ...
        for command in commands:
            with patch("sys.argv", ["run_pipeline.py", command]):
                with patch("sys.exit"):
                    ...
                    assert (
                        execution_time < 1.0
                    ), f"Comando {command} muito lento: {execution_time}s"
    ...
    def test_memory_usage(self, mock_pipeline_class, mock_pipeline):
        ...
        try:
            ...
            ), f"Uso excessivo de memória: {memory_increase} bytes"
        except ImportError:
    ...
    def test_concurrent_execution(self, mock_pipeline_class, mock_pipeline):
        ...
        while not results.empty():
            ...
            assert (
                execution_time < 1.0
            ), f"Comando {command} muito lento em execução concorrente: {execution_time}s"
    ...
    def test_large_argument_handling(self, mock_pipeline_class, mock_pipeline):
        ...
        with patch("sys.argv", large_args):
            with patch("sys.exit"):
                ...
                assert (
                    execution_time < 1.0
                ), f"Execução muito lenta com muitos argumentos: {execution_time}s"
